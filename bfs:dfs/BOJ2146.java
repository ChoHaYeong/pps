       ...
       // map[x][y] = idx;
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i=0; i<4; i++) {
                ...
                // System.out.println( " out.x " + out.x + " out.y " + out.y + "dist[out.x][out.y] :" + dist[out.x][out.y] );
                // System.out.println( " nx " + nx + " ny " + ny + "dist[nx][ny] :" + (dist[out.x][out.y]+1) );

                if(nx<0 || ny<0 || nx >=N || ny >=N) continue;
                else if(visited[nx][ny]) continue; //바다와 인접해있지 않는 경우에는 continue 는 경우만 ..
